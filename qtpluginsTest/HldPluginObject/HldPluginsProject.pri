...
if(contains(DEFINES,HLD_PLUGIN)){
    CONFIG(debug, debug|release){
        DEFINES += _BUILD_TYPE_DEBUG_
        COMPILE_TARGET = Debug
        __OUT_NAME__ = $${EXE_APP_NAME}D
...
unix {
    ...
    if(contains(DEFINES,HLD_PLUGIN)){
        ...
        # 您可以自定义插件的生成位置，如果不定义，那么则默认生成在执行目录下的hldPlugins文件夹下
        # 格式不能错,如下书写，不能有多余的空格，谢谢！

        # DEFINES += HLD_PLUGIN
        # EXE_APP_NAME = your plugin name
        # DEFINES += HLD_PLUGIN_DESTDIR
        # HLD_PLUGIN_DESTDIR=$${your dirname}
        # include(../HldPluginsProject.pri)


        if(contains(DEFINES,HLD_PLUGIN_DESTDIR)){
            DESTDIR = $$PWD/../../$${PLATFORM}/$${HLD_PLUGIN_DESTDIR}
        }else{
            DESTDIR = $$PWD/../../$${PLATFORM}/hldPlugins
        }
    }
...
win32 {
    ...
    if(contains(DEFINES,HLD_PLUGIN)){
        # 这种方式只生成一种dll文件，因为它是插件形式，加载的方式也不一样
        TEMPLATE = lib
        CONFIG += plugin
